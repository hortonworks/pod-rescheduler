...
func (f *TimeFormatter) Format(entry *log.Entry) ([]byte, error) {
...
func printColored(b *bytes.Buffer, entry *log.Entry, keys []string) {
	...
	if runtime.GOOS == "windows" {
		...
		fmt.Fprintf(b, "\x1b[%dm%-6s\x1b[0m %s %-44s ", levelColor, levelText, entry.Time.Format(time.RFC3339), entry.Message)
		for _, k := range keys {
			v := entry.Data[k]
...
func (f *TimeFormatter) appendKeyValue(b *bytes.Buffer, key, value interface{}) {
	switch value.(type) {
	case string, error:
